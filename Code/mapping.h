...
const byte MAP_RESOLUTION = 25;//we can change the map resolution
const byte MAP_DEFAULT_FEATURE = '#';
const int MAP_X=1800;//1800
...
class Mapper
{
    public:
        ...
        char returnPosition(int y, int x);
...
void Mapper::printMap()//25*25 grid -- scaled down to 7.2*7.2 cm
{
...
{
      if (x > MAP_X || x < 0 || y > MAP_Y || y < 0)//change lower/upper limit used to be 0/1800
    {
...
char Mapper::returnPosition(int y, int x){//return the character in a particular position on the map. - written by Ranvir
